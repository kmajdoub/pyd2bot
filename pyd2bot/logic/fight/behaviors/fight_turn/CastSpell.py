...
class CastSpell(AbstractBehavior):
    ...
    def __init__(self, spellw: SpellWrapper, target_cellId: int):
        super().__init__()
        self.spellw = spellw
    ...
    def castSpell(self) -> None:
        Logger().info(f"Casting spell {self.spellw.spellId} on cell {self.target_cellId}")
        ...
        if not self._cast_spell_request_sent:
            canCast, reason = can_cast_spell_on_cell(self.spellw, self.target_cellId)
            if canCast:
                has_los, los_reason = check_line_of_sight(fighterPos.cellId, self.target_cellId)
                if not has_los:
                    Logger().error(f"Can't cast spell {self.spellw.spellId} on cell {self.target_cellId}: {los_reason}")
    ...
    def send_cast_spell_request(self):
        ...
        message.init(self.spellw.spellId, self.target_cellId)
    ...
    def on_spell_casted(self, event, sourceId, destinationCellId, sourceCellId, spellId):
        if sourceId == self.state_manager.current_player.id and self.target_cellId == destinationCellId:
            ...
            if self._cast_spell_request_sent:
                Logger().info(f"Spell {self.spellw.spell.name} casted successfully!")
                self.finish(0)
            else:
