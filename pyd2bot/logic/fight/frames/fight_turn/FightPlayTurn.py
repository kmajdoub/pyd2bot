...
from pydofus2.com.ankamagames.dofus.network.messages.game.context.fight.GameFightTurnFinishMessage import GameFightTurnFinishMessage
...
class FightPlayTurn(AbstractBehavior):
    ...
    def run(self) -> bool:
        ...
        # Set up sequence watching and initialize turn
        # self.watch_fight_sequence(lambda *_: self.next_action())
    ...
    def _initialize_turn(self) -> None:
        ...
        # self.on(KernelEvent.FightTurnEnd, self.on_turn_ended)
        ...
        if not self.state_manager.validate_turn_state():
            if not self.state_manager.player_manager:
                self._handle_disconnected_player()
            else:
                self.finish(TurnResult.INVALID_STATE, "Invalid turn state")
            return

        # Prepare fight state if not already done
        self.state_manager.prepare_turn_state()
            
        Logger().info(f"It's {self.state_manager.current_player.name}'s turn to play")
        self.check_can_play_turn()

    def check_can_play_turn(self, event=None) -> None:
        """Validate turn conditions and start main logic"""
        if not (Kernel().turnFrame and 
                Kernel().turnFrame.myTurn and 
                self.state_manager.validate_turn_state()):
            return self.finish(TurnResult.INVALID_STATE, f"Turn conditions not met, turn frame={Kernel().turnFrame}, my turn={Kernel().turnFrame.myTurn}")
    ...
    def main(self) -> None:
        ...
        if not self._can_cast_spells():
            return self._end_turn(
                TurnResult.CANNOT_CAST,
                "Unable to cast spells"
            )

        targets = self._find_valid_targets()
        if not targets:
            return self._end_turn(
                TurnResult.NO_TARGETS,
                "No valid targets found"
            )

        self.state_manager.log_turn_stats()
        self._plan_turn_actions(targets)
        self.next_action()

    # === Action Management ===
    def _plan_turn_actions(self, targets: List["GameFightFighterInformations"]) -> None:
        """Plan sequence of actions for the turn"""
        ...
        if path is None:
            Logger().debug(f"Didnt find any path to targets!")
            return self._end_turn(
                TurnResult.NO_PATH,
    ...
    # === Helper Methods ===
    def _handle_disconnected_player(self) -> None:
        ...
        self.finish(TurnResult.DISCONNECTED, "Player disconnected")

    def _can_cast_spells(self) -> bool:
        """Check spell casting ability"""
        can_cast, reason = can_cast_spell_on_cell(
            self.state_manager.spellId,
            self.state_manager.spellw.spellLevel
        )
        if not can_cast:
            Logger().info(f"Unable to cast spells: {reason}")
        return can_cast

    def _find_valid_targets(self) -> Optional[List["GameFightFighterInformations"]]:
        """Find valid spell targets"""
        filters = [
            (True, 2672),
            (True, 91)
        ] if self.state_manager.session.isTreasureHuntSession else [
            (False, None),
            (True, None)
        ]
